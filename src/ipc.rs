...
pub async fn start_ipc_server<F, G>(toggle_callback: F, reload_callback: G) -> std::io::Result<()>
where
    F: Fn() + Send + 'static + Clone,
    G: Fn() + Send + 'static + Clone,
{
...
pub async fn send_reload_command() -> std::io::Result<()> {
    let socket_path = get_socket_path();
    ...
    // Write with timeout
    tokio::time::timeout(Duration::from_millis(200), stream.write_all(b"reload"))
        .await
        .map_err(|_| {
            std::io::Error::new(std::io::ErrorKind::TimedOut, "Write operation timed out")
        })??;
